...
// GameContainer peut utiliser n'importe quel composant
const GameComponent = getGameComponent(gameType)
return <GameComponent data={gameData} onAnswer={handleAnswer} onComplete={handleComplete} />
```
...
function App() {
  return (
    <AuthProvider>        {/* Abstraction: authentification */}
      <SocketProvider>    {/* Abstraction: communication temps réel */}
        <RouterProvider>  {/* Abstraction: navigation */}
          {/* Les composants dépendent des abstractions */}
        </RouterProvider>
      </SocketProvider>
    </AuthProvider>
  )
...
// Les composants dépendent du contexte (abstraction)
function MyComponent() {
  const { user } = useAuth()  // Abstraction
  // Le composant ne sait pas si les données viennent de localStorage, API, etc.
}
...
## Patrons de Conception GoF Détaillés
...
| ----------------------------------------------- | ------ | ------------------------------------------------- |
| **Manuel d'utilisation de l'application** | ✅     | README.md (section "Guide d'Utilisation")         |
| **5 Principes SOLID avec exemples**       | ✅     | README.md (section "Principes SOLID Détaillés") |
| **3 Patrons GoF avec diagrammes**         | ✅     | README.md (section "Patrons GoF Détaillés")     |
| **Tests unitaires (25 tests)**            | ✅     | `Tp_play_learn-Public_back/tests/` + README.md  |
| **Code source frontend**                  | ✅     | `src/` (React + Vite)                           |
| **Code source backend**                   | ✅     | `Tp_play_learn-Public_back/src/` (Node.js)      |
| **Documentation technique complète**     | ✅     | 3 READMEs (Global, Frontend, Backend, Tests)      |
| **Architecture full-stack fonctionnelle** | ✅     | Frontend + Backend + Base de données MySQL       |

### Contenu des Livrables

#### 1. **Manuel d'Utilisation** ✅

- Installation complète (MySQL, Backend, Frontend)
- Guide utilisateur étape par étape (8 sections)
- Flux d'utilisation avec diagrammes
- Raccourcis clavier
- Troubleshooting (5 problèmes courants)

#### 2. **Principes SOLID** ✅

- **S** - Single Responsibility : Contextes séparés, Services spécialisés
- **O** - Open/Closed : GameContainer extensible, Routes modulaires
- **L** - Liskov Substitution : Composants de jeu interchangeables
- **I** - Interface Segregation : Contextes spécialisés, Props ciblées
- **D** - Dependency Inversion : Injection via Context API, Configuration externalisée

**Exemples de code concrets** : 15+ extraits de code réels du projet

#### 3. **Patrons de Conception GoF** ✅

- **Observer** : Context API + Socket.io (AuthContext, SocketContext)
- **Strategy** : Game types, Score calculation, Validation
- **Singleton** : API Axios, Socket connection, Game state manager

**Diagrammes UML** : 3 diagrammes illustrant les patterns

#### 4. **Tests Unitaires** ✅

- **25 tests** répartis en 4 fichiers :
  - `auth.test.js` : 4 tests (inscription, connexion, token, invité)
  - `games.test.js` : 4 tests (CRUD, filtres, catégories)
  - `multiplayer.test.js` : 4 tests (salles, rejoindre, socket)
  - `advanced.test.js` : 13 tests (score, leaderboard, buzzer, timer)

**Résultats** : ✅ 25 passed (6.11s) - Tous les tests passent !

**Coverage** :

- Authentification : 100%
- Gestion des jeux : 100%
- Multijoueur : 100%
...
Voir le README détaillé : [`FRONTEND_README.md`](FRONTEND_README.md)

- Installation et configuration détaillée
- Structure complète des composants
- Context API et hooks personnalisés
- Services et API integration
- Styling et système de thème
- Guide de contribution complet

### **Backend**

Voir le README détaillé : [`Tp_play_learn-Public_back/README.md`](Tp_play_learn-Public_back/README.md)

- Installation MySQL complète (XAMPP/WAMP/MAMP par OS)
- Création de la base de données étape par étape
- Documentation des 25 endpoints REST API
- Modèles Sequelize et relations
- Middleware et authentification JWT détaillée
- Documentation Socket.io events
- Troubleshooting approfondi (10+ problèmes)

###  **Tests Unitaires**

Voir le README détaillé : [`Tp_play_learn-Public_back/tests/README.md`](Tp_play_learn-Public_back/tests/README.md)

- Description détaillée des 25 tests
- Coverage par catégorie avec statistiques
- Résultats des tests avec sorties complètes
- Commandes de test (watch, coverage)
...
- **Frontend** : ESLint avec configuration React
...
Ce projet est développé dans un cadre éducatif pour le cours **INF1011 - Génie Logiciel** à l'Université du Québec à Trois-Rivières (UQTR).

---

## Auteurs

**Play&Learn Team - Automne 2025**

Développé avec  par l'équipe INF1011
...
2. Consultez les READMEs détaillés (Frontend, Backend, Tests)
3. Vérifiez les issues GitHub existantes
4. Ouvrez une nouvelle issue avec le tag approprié

---

## Contexte Académique

**Cours** : INF1011 - Génie Logiciel
**Professeur** : Mr William Flageol
**Session** : Automne 2025
**Université** : UQTR (Université du Québec à Trois-Rivières)

**Objectifs d'Apprentissage Atteints** :

- ✅ Conception orientée objet avec principes SOLID
- ✅ Patrons de conception (GoF)
 ...
 **Play&Learn - Apprendre en s'amusant !**
